...
#' plot_alignment_heatmap(PAC)
#'
#' @note The `plot_category_matrix` displays the dissimilarity categories for all characters in the reference alignment ($dissimilarity_simple). This gives a representation of which columns are well agreed upon by the MSAs, and which columns of the reference alignment are split, merged, or shifted.
#'
plot_category_matrix <- function(x,display=TRUE){
  ...
  md           <- reshape2::melt(hm_data)
  colnames(md) <- c('Position','Sequence','Category')


  p <- ggplot2::ggplot(md)                                                   +
       ggplot2::geom_tile(ggplot2::aes(x=Position,y=Sequence,fill=Category)) + 
       ggplot2::scale_x_continuous(expand = c(0, 0))                         +
